...
import { 
  ...
  Lock, Share2, Copy, Plus, HelpCircle, Activity, Bell, MessageCircle, Bot, Menu, X
} from 'lucide-react';
...
export default function UserDashboard({ user, onNavigateToPlans }: UserDashboardProps) {
  ...
  return (
    <div id="user-dashboard-wrapper" className="min-h-screen text-gray-200">
      ...
      <div className="md:hidden flex items-center justify-between bg-[#040212]/95 backdrop-blur-md border-b border-purple-500/10 px-4 py-3 sticky top-0 z-40">
        ...
        <span className="text-xs font-mono font-black tracking-wider uppercase text-purple-400 bg-purple-950/20 border border-purple-500/10 px-2.5 py-1 rounded-lg">
          Investor Desk
        </span>
        <button 
        ...
        <div className="hidden md:block w-64 shrink-0">
          <div className="sticky top-24 bg-[#05041a]/60 backdrop-blur-md rounded-2xl border border-purple-500/10 p-4 space-y-4 shadow-xl shadow-black/40">
            <div className="px-2 py-1 border-b border-white/5 pb-3">
              <h3 className="text-xs font-mono font-black tracking-widest text-purple-400 uppercase">
                Navigation Hub
              </h3>
              <p className="text-[10px] text-gray-500 mt-1">Select operations node</p>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    {item.badge !== undefined && item.badge > 0 && (
                      <span className="px-1.5 py-0.5 rounded-md text-[10px] font-mono bg-red-500/20 border border-red-500/30 text-red-400 font-black">
                        {item.badge}
                      </span>
                    )}
                  </button>
            ...
            <>
              ...
              >
                ...
                <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-4">
                  <div>
                    <h3 className="text-sm font-bold text-white tracking-tight">Investor Menu</h3>
                    <p className="text-[10px] text-purple-400 font-mono">ID: {user.email.split('@')[0]}</p>
                  </div>
                  <button
                    onClick={() => setIsSidebarOpen(false)}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <Icon className={`h-4.5 w-4.5 ${isActive ? 'text-purple-400' : 'text-gray-500'}`} />
                          <span>{item.label}</span>
                        </div>
                        {item.badge !== undefined && item.badge > 0 && (
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-mono bg-red-500/20 border border-red-500/30 text-red-400 font-bold">
                            {item.badge}
                          </span>
                        )}
                      </button>
                ...
                <div className="border-t border-white/5 pt-4 mt-4">
                  <div className="flex items-center gap-3 bg-black/40 px-3 py-2 rounded-xl border border-white/5">
                    ...
                    <div>
                      <p className="text-[8px] text-gray-500 uppercase tracking-wider font-semibold">Crypto Insurance</p>
                      <p className="text-[10px] font-bold text-emerald-400 font-mono">Escrow Secured</p>
                    </div>
                  </div>
                </div>
              </motion.div>
